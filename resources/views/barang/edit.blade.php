        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="glass-card overflow-hidden shadow-sm sm:rounded-lg fade-in">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('barang.update', $barang) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            ...
                            <x-text-input id="nama" class="block mt-1 w-full glass-card border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500 transition-all duration-300" type="text" name="nama" :value="old('nama', $barang->nama)" required autofocus />
                        </div>

                        <!-- Kategori -->
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            <x-input-label for="kategori" :value="__('Kategori')" class="text-gray-900 dark:text-gray-100 font-semibold" />
                            <x-text-input id="kategori" class="block mt-1 w-full glass-card border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500 transition-all duration-300" type="text" name="kategori" :value="old('kategori', $barang->kategori)" required />
                        </div>

                        <!-- Stok -->
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            <x-input-label for="stok" :value="__('Stok')" class="text-gray-900 dark:text-gray-100 font-semibold" />
                            <x-text-input id="stok" class="block mt-1 w-full glass-card border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500 transition-all duration-300" type="number" name="stok" :value="old('stok', $barang->stok)" required />
                        </div>

                        <!-- Harga -->
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            <x-input-label for="harga" :value="__('Harga')" class="text-gray-900 dark:text-gray-100 font-semibold" />
                            <x-text-input id="harga" class="block mt-1 w-full glass-card border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500 transition-all duration-300" type="number" name="harga" :value="old('harga', $barang->harga)" required step="1" />
                        </div>

                        <!-- Deskripsi -->
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            <x-input-label for="deskripsi" :value="__('Deskripsi')" class="text-gray-900 dark:text-gray-100 font-semibold" />
                            <textarea id="deskripsi" name="deskripsi" rows="4" class="block mt-1 w-full glass-card border-gray-300 dark:border-gray-600 dark:bg-gray-900/50 dark:text-gray-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm transition-all duration-300">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                        </div>

                        <!-- Foto -->
                        <div class="transform transition-all duration-300 hover:scale-[1.01]">
                            <x-input-label for="foto" :value="__('Foto Barang (Opsional)')" class="text-gray-900 dark:text-gray-100 font-semibold" />
                            
                            @if ($barang->foto)
                                <div class="mt-2 mb-4 glass-card p-4 rounded-lg bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/30 dark:to-purple-800/30 border border-purple-200 dark:border-purple-700/50">
                                    ...
                                    <div class="relative inline-block group">
                                        <img src="{{ asset('storage/' . $barang->foto) }}" alt="{{ $barang->nama }}" class="w-32 h-32 object-cover rounded-lg shadow-lg hover-lift border-2 border-purple-300 dark:border-purple-700">
